...
// Each MCP session gets its own McpServer instance, so tools are registered
// via this function rather than on a single global server.
...
function createSessionServer(): McpServer {
  const s = new McpServer({ name: "google-forms", version: "1.0.0" });
  registerTools(s);
  return s;
}
